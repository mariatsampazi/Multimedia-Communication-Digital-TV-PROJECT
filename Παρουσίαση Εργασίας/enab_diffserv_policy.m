...
% standards
MAX_PR_PERC = 0.5;   % Priority 3
MED_PR_PERC = 0.2;   % Priority 2
MIN_PR_PERC = 0.3;   % Priority 1
...
while (time<MAX_time)
    ...
    if (link_1_Trans==1)
        ...
        if(push==0 && link_1_PR==3)
            [find,index]=findIndex(BUFFER_1,B1_CS,BUFFER_1_size);
            if(index~=0)
    ...
    if (link_2_Trans==1)
        ...
        if(push==0 && link_2_PR==3)
            [find,index]=findIndex(BUFFER_2,B2_CS,BUFFER_2_size);
            if(index~=0)
